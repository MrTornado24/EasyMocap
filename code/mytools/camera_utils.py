...
def read_camera(intri_name, extri_name, cam_names=[0,1,2,3]):
    ...
    for cam in cam_names:
        # 内参只读子码流的
        cams[cam] = {}
        cams[cam]['K'] = intri.read('K_{}'.format(cam.split('_')[2]))
        cams[cam]['invK'] = np.linalg.inv(cams[cam]['K'])
        # Rvec = extri.read('R_{}'.format(cam)) # jingxiang
        R = extri.read('R_{}'.format(cam.split('_')[2]))
        Tvec = extri.read('T_{}'.format(cam.split('_')[2])).T
        ...
        cams[cam]['dist'] = intri.read('dist_{}'.format(cam.split('_')[2]))
...
def write_camera_aist(camera, path):
    from os.path import join
    intri_name = join(path, 'intri.yml')
    extri_name = join(path, 'extri.yml')
    intri = FileStorage(intri_name, True)
    extri = FileStorage(extri_name, True)
    results = {}
    camnames = [key['name'] for key in camera]
    intri.write('names', camnames, 'list')
    extri.write('names', camnames, 'list')
    for val in camera:
        key = val['name']
        intri.write('K_{}'.format(key), np.asmatrix(val['matrix']))
        intri.write('dist_{}'.format(key), np.asmatrix(val['distortions']))
        if 'Rvec' not in val.keys():
            val['Rvec'] = cv2.Rodrigues(np.asmatrix(val['rotation']))[0]
        extri.write('R_{}'.format(key), np.asmatrix(val['Rvec']))
        extri.write('Rot_{}'.format(key), np.asmatrix(val['rotation']))
        extri.write('T_{}'.format(key), np.asmatrix(val['translation']))
...
def main():
    path = '/raid/sjx/EasyMocap/aist_plusplus_final/dataset/gBR_sBM_cAll_d04_mBR0_ch01'
    with open('/raid/sjx/EasyMocap/aist_plusplus_final/dataset/gBR_sBM_cAll_d04_mBR0_ch01/setting1.json', 'r') as f:
            camera = json.load(f)
    print(camera[0])
    write_camera_aist(camera, path)
